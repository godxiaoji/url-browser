...
const URLBrowser = require('../dist/url-browser')

describe('#myUrl = new URLBrowser("https://test.com/path?a=1&b=2#hash")', () => {
  const url = 'https://test.com/path?a=1&b=2#hash'

  describe('returns', () => {
    const myUrl = new URLBrowser(url)
  ...
  describe('addQuery', () => {
    it('myUrl.addQuery("a", 1) 增加单个参数', () => {
      assert.strictEqual(
        new URLBrowser(url).addQuery('c', '测试').toString(),
    ...
    it('myUrl.addQuery({"c": 3, "d": 4}) 增加多个参数', () => {
      assert.strictEqual(
        new URLBrowser(url).addQuery({ c: 3, d: 4 }).toString(),
    ...
    it('myUrl.addQuery("e", {"a": 1, "b": 2}) 参数值是个对象，会进行序列化', () => {
      assert.strictEqual(
        new URLBrowser(url).addQuery('e', { a: 1, b: 2 }).toString(),
  ...
  describe('removeQuery', () => {
    it('myUrl.removeQuery("b") 删除单个参数', () => {
      assert.strictEqual(
        new URLBrowser(url).removeQuery('b').toString(),
    ...
    it('myUrl.removeQuery(["a", "b"]) 删除多个参数', () => {
      assert.strictEqual(
        new URLBrowser(url).removeQuery(['a', 'b']).toString(),
  ...
  describe('setHash', () => {
    it('myUrl.setHash("#hash2") 设置hash', () => {
      assert.strictEqual(
        new URLBrowser(url).setHash('#hash2').toString(),
    ...
    it('myUrl.setHash("hash3") 支持不加#号', () => {
      assert.strictEqual(
        new URLBrowser(url).setHash('hash3').toString(),
    ...
    it('myUrl.setHash(null) 设置为空', () => {
      assert.strictEqual(
        new URLBrowser(url).setHash(null).toString(),
